...
async def check_btb_file(
    ...
    Returns:
        The spec name (first line of the .btb file, stripped) if a .btb file
        is found, or None if no .btb file exists.
...
async def handle_webhook(request: web.Request) -> web.Response:
    ...
    Validates the webhook signature, extracts push payload data,
    ...
    try:
        async with aiohttp.ClientSession() as session:
            spec_name = await check_btb_file(session, owner, repo, commit_sha, github_token)
    except Exception as e:
        logger.error("GitHub API error checking .btb file: %s", e)
        return web.Response(status=500, text="GitHub API error")

    if spec_name is None:
        logger.debug("No .btb file found in %s at %s", full_name, commit_sha)
        return web.Response(status=200, text="Skipped: no .btb file")
